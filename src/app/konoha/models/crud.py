...
class Guild(CRUDBase):
    ...
    async def set(self, verbose=1, **kwargs):
        q = models.guild.update(None).where(self.guild_id == models.guild.c.id).values(**kwargs)
...
class Hook(CRUDBase):
    ...
    async def get(self, verbose=1):
        q = models.hook.select().where(and_(self.guild_id == models.hook.c.guild, self.channel_id == models.hook.c.channel))
        result = await Hook.execute(q, verbose)
        return await result.fetchone()

    async def set(self, verbose=1, **kwargs):
        q = models.hook.update(None).where(and_(self.guild_id == models.hook.c.guild, self.channel_id == models.hook.c.channel)).values(**kwargs)
        await Hook.execute(q, verbose)
        return self

    async def delete(self, verbose=1):
        q = models.hook.delete(None).where(and_(self.guild_id == models.hook.c.guild, self.channel_id == models.hook.c.channel))
        await Hook.execute(q, verbose)
        return self

    @classmethod
    async def create(cls, guild_id, channel_id, hookurl, tags, verbose=1):
        q = models.hook.insert(None).values(guild=guild_id, channel=channel_id, hookurl=hookurl, tags=tags)
    ...
    async def search(verbose=1, **kwargs):
        q = models.hook.select().where(and_(*[v == getattr(models.hook.c, k) for k, v in kwargs.items()]))
        result = await Hook.execute(q, verbose)
...
class Reminder(CRUDBase):
    ...
    async def list(self, verbose=1):
        c = await self.get_config(verbose)
        q = models.reminder.select().order_by(models.reminder.c.start_at)\
            .where(
                and_(
                    models.reminder.c.config == c.id,
                    datetime.now().astimezone(timezone('Asia/Tokyo')) <= models.reminder.c.start_at
                )
            )
        result = await Reminder.execute(q, verbose)
        return await result.fetchall()
    ...
    async def search_between_minutes(minutes, verbose=1):
        q = models.reminder.select().where(
            and_(
                datetime.now().astimezone(timezone('Asia/Tokyo')) + timedelta(minutes=minutes-1) <= models.reminder.c.start_at,
                datetime.now().astimezone(timezone('Asia/Tokyo')) + timedelta(minutes=minutes) >= models.reminder.c.start_at,
            )
        )
        result = await Reminder.execute(q, verbose)
        return await result.fetchall()
